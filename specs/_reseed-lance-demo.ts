// Throwaway reseed at 1024 dims to match the qwen 0.6b embedder profile used for screenshot
// capture. See the original _seed-lance-demo.ts design notes (deleted after first use); this is
// the same generator with DIM bumped. Delete when screenshot capture is done.
import * as lancedb from "@lancedb/lancedb";

const outDir = process.argv[2];
if (!outDir) {
  console.error("usage: bun run specs/_reseed-lance-demo.ts <outDir>");
  process.exit(1);
}
...
const TOPICS = [
  {
    category: "Billing",
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.sin(i * 0.031) * 3),
    templates: [
  ...
  {
    category: "Login / Auth",
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.cos(i * 0.047) * 3 + 2),
    templates: [
  ...
  },
  {
    ...
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.sin(i * 0.019 + 1) * 3 - 2),
    templates: [
  ...
  },
  {
    ...
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.cos(i * 0.061 + 2) * 3 + 4),
    templates: [
  ...
  {
    category: "Mobile App",
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.sin(i * 0.083 + 3) * 3 - 4),
    templates: [
  ...
  },
  {
    ...
    centroid: () => Array.from({ length: DIM }, (_, i) => Math.cos(i * 0.029 + 4) * 3),
    templates: [
...
console.log(`Reseeded ${rows.length} rows (${TOPICS.length} topics, dim=${DIM}) into ${outDir}/support_tickets.lance`);
