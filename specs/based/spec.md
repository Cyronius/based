...
**Linux keyring availability.** The Secret Service is reached over the session D-Bus, which is not guaranteed to exist (WSL2, headless CI, a bare TTY login). Availability shall be decided **before** any native keyring call: `DBUS_SESSION_BUS_ADDRESS` unset (or empty) with no `$XDG_RUNTIME_DIR/bus` socket means unavailable. `BASED_KEYRING=off` forces the unavailable state on any platform — the operator's escape hatch, because a bus can exist with no Secret Service behind it (WSL2 with systemd) and the bus check cannot see that: probing the service is itself the crashing call.

**Plaintext fallback (keyring unavailable).** Secrets still save — into the `secrets_fallback` table in app.db, unencrypted, **loudly**: the UI warns above every secret input *before* the user types (`GET /api/secret-store` → `{ backend: "keyring" | "plaintext", reason }`, rendered by `SecretStoreWarning`), and core logs one warning per plaintext write. No app-embedded encryption — that would be obfuscation wearing a security smell. Reads consult the keyring first (when available) and fall through to the fallback, so a key saved keyring-less still resolves once the machine gains a keyring; the next write with a working keyring stores to the keyring and purges the fallback row (upgrade-in-place, the same stance as the v2-marker migration). Deletes clear both stores. The 2560-byte cap applies to the fallback too — it is a portability rule, not a keyring rule. See [archive/plaintext-secret-fallback.md](archive/plaintext-secret-fallback.md). A pre-check rather than a catch, because `@napi-rs/keyring` does not throw in this state — it **segfaults the Bun process** (observed Bun 1.3.14 on WSL2 Ubuntu 24.04; found when saving an AI profile crashed core mid-request), and no try/catch contains a SIGSEGV. The decision function `keyringUnavailableReason(platform, env, socketExists)` is pure so every branch is unit-testable from any host.
...
- `keyringUnavailableReason` on win32/darwin → null; on linux with `DBUS_SESSION_BUS_ADDRESS` set or a `$XDG_RUNTIME_DIR/bus` socket → null; on linux with neither (or an empty bus address) → a reason naming D-Bus; `BASED_KEYRING=off` → a reason naming the variable, on every platform and regardless of bus state (covered by `unit.keyringGuard.test.ts`)
- Keyring unavailable: `setSecret` → `getSecret` round-trips via `secrets_fallback` and the raw row is the plaintext value; `deleteSecret` removes it; an over-cap secret is refused naming the limit and stores nothing (covered by `integration.secretsFallback.test.ts`)
- Keyring available: a pre-seeded fallback row reads back through `getSecret`, and the next `setSecret` moves the value to the keyring and purges the row (covered by `integration.secretsFallback.test.ts`)
- `GET /api/secret-store` reports `plaintext` plus the reason when the keyring is unavailable (covered by `integration.secretsFallback.test.ts`); the pre-save UI warning is `manual`: open an editor with `BASED_KEYRING=off`, see the warning before saving, save a key, restart, the key still works
